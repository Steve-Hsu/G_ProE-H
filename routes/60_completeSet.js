...
router.post('/', authUser, async (req, res) => {
   ...
   if (!newCsOrder) {
      return res.status(400).json({ msg: 'No csOrder posted' });
   }
   //Check if the user have the right
   //Here set the right for case, the reason relates to the management science and my management philosophy.
   if (!user.cases) {
   ...
   if (completeSet) {
      ...
      const checkTheOrder = checkOrderArr.length === newCsOrder.length ? true : false;
      const compareLtOsConfirmDateBetweenOSandCS =
         String(orderSummary.osLtConfirmDate) === String(completeSet.osLtConfirmDate) ? true : false;

      if (checkTheOrder && compareLtOsConfirmDateBetweenOSandCS) {
         // the order of array not changed, or return the original cs.
         console.log('The osUpdate date are same and the CsList is not changed so, no need to update, the completeSet is returned')
         return res.json(completeSet);
      } else {
         ...
         const countCases = new Promise(async (resolve) => {
            newCsOrder.map(async (newCsOrderCNo, idx) => {
               ...
               const countMtrls = new Promise(async (resolve) => {
                  ...
                  // Default the leadTime in gQty
                  const defaultTheDateIngQty = currentCase.gQtys.map((gQty) => {
                     new Promise((resolve) => {
                        gQty.leadTime = null
                        resolve()
                     })
                  ...
                  Promise.all(defaultTheDateIngQty).then(() => {
                     theMtrls.map((mtrl, idx) => {
                        ...
                        const countCSPTs = new Promise(async (resolve) => {
                           // The set with high gQty will be priority to fulfill with it materials
                           // This for production to be moer save when a new style put in production, factory prefers the set, the size and cWay, with more quanity.
                           const sortedgQtys = currentCase.gQtys.sort((a, b) => {
                              if (a.gQty < b.gQty) {
                                 return 1;
                              }
                              if (a.gQty > b.gQty) {
                           ...
                           sortedgQtys.map((sTgQty, idx) => {
                              //Add 2 new attribute for cspt in CS and give it default value.
                              let cspt = mtrl.cspts.find(({ gQty }) => gQty === sTgQty.id)
                              ...
                              const checkTheValueCspt = new Promise((resolve) => {
                                 cspt.distriputedQty = 0
                                 resolve()
                              })

                              Promise.all([checkTheValueCspt]).then(() => {
         ...
         Promise.all([countCases]).then(async () => {
            ...
            const updateCs = await CS.findOneAndUpdate(
               ...
               {
                  $set: {
                     osLtConfirmDate: orderSummary.osLtConfirmDate,
                     csOrder: newCsOrder,
                     caseMtrls: osMtrls,
            ...
            // console.log("the checkTheOrder", checkTheOrder)
            // console.log("the compareLtOsConfirmDateBetweenOSandCS", compareLtOsConfirmDateBetweenOSandCS)
            // console.log("updateCs is returned")
            // console.log("orderSummary.osLtConfirmDate", orderSummary.osLtConfirmDate)
            // console.log("orderSummary.osLtConfirmDate type", typeof orderSummary.osLtConfirmDate)
            // console.log("completeSet.osLtConfirmDate", completeSet.osLtConfirmDate)
            // console.log("completeSet.osLtConfirmDate type", typeof completeSet.osLtConfirmDate)
            // const lastCheck = String(orderSummary.osLtConfirmDate) === String(completeSet.osLtConfirmDate) ? true : false;
