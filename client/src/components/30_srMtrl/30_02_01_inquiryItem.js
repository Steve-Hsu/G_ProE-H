...
const InquiryItem = ({ srMtrl, mPrice, idx, className, inquirySupplier }) => {
    return (
        <div className={`grid-Inquiry-Mtrl m-0 p-0 bd-light bd-no-t ${className}`}>
            ...
            {/* Ref_no */}
            <div className='bd-light bd-no-t v-center-content px-05'>{!inquirySupplier ? `${srMtrl.supplier} / ${srMtrl.ref_no}` : srMtrl.ref_no}</div>
            {/* Description */}
            <div className='bd-light bd-no-t v-center-content px-05'>{
                mPrice.sizeSPEC && mPrice.mColor ? `${mPrice.sizeSPEC} | ${mPrice.mColor}` :
                    mPrice.sizeSPEC || mPrice.mColor}
            </div>
            {/* Unit Price */}
